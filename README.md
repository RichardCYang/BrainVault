...
Run the normal dependency installation and database migration workflow (`npm ci`, `npm run db:migrate`, `npm run build`) before restarting the application. Migration `076_mfa_step_up_sessions.sql` creates short-lived MFA step-up state, and migration `077_custom_icon_tenant_identity.sql` changes custom-icon identity from a global key to a tenant-scoped composite key. Automatic database bootstrap applies pending migrations when enabled; otherwise apply migrations explicitly before starting this version.

API clients that add, rename, replace, or remove MFA factors, and clients that change the account password, must include a valid `stepUpToken` when the account already has an enrolled factor. The browser obtains that token by confirming an existing TOTP code or passkey. A successful password change intentionally removes existing TOTP/passkey credentials so they must be enrolled again. Existing attachment download defenses remain in place. The archive retains the original project paths and Git metadata and adds no audit-log or report-log files.

### Validation and limits

- Browser JavaScript syntax checks passed for `public/app.js` and `public/i18n.js`.
- All modified TypeScript sources passed TypeScript syntax/transformation parsing with `tsc --noCheck`; a full project type-check could not be executed without installed project dependencies.
- Focused source-level Node security tests covering the affected registration, TOTP-IP, and custom-icon controls passed 32/32.
- The attachment filename hardening was executed directly after TypeScript transpilation: `payload.html::$DATA` canonicalizes to `payload.html` and receives the `.download` hardening suffix; `payload.svg:ads` behaves equivalently.
- The packaging environment could not resolve `registry.npmjs.org`, so `npm ci`, the full `npm run verify:security` command, Vitest suites, and a dependency lockfile refresh were not executed here. The existing lockfile is unchanged.
