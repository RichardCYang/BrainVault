...
The VPN/proxy access policy intentionally runs on every authenticated HTTP request. A successful clear-risk result is cached for five minutes. The VPN Gate public-relay directory is also refreshed every five minutes and can be as large as 8 MiB with a four-second network timeout.

Before this change, `matchVpnGateRelay()` synchronously awaited the full VPN Gate directory refresh whenever its five-minute freshness window expired. That means the first authenticated request after an idle period could inherit the bulk threat-intelligence download latency even though BrainVault already had a previously successful directory snapshot that its own safety policy permits retaining for up to fifteen minutes. The same request path also synchronously refreshed the Tor bulk list after its hourly freshness window, despite an existing six-hour stale-safety window.
...
`tests/vpngate-refresh-latency.node.test.mjs` reproduces the timing boundary with a controlled clock and a deliberately unresolved second directory fetch. It verifies that after the five-minute freshness window expires:
