...
Security details for server-side preview fetching are documented in [Security](security.md#bookmark-preview-safety).
...
Attachment storage and deletion rules are documented in [Security](security.md#attachment-safety).
...
The selection is saved to the account and to `localStorage` under `brainvault.language`. Translation implementation details are in [Development](development.md#translations).
