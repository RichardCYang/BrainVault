...
Current-format backups store page sharing grants by the collaborator's normalized login ID. Restore resolves and locks those accounts before deleting any page; if a referenced collaborator does not exist, the operation fails closed and no workspace data is replaced. Archived ordinary pages may retain grants: archive blocks live collaboration, while backup and restore preserve the access list so unarchiving does not silently lose collaborators. Legacy backups without a `pageShares` field preserve the account's current grants for matching imported ordinary page IDs, including archived pages, preventing the page deletion cascade from silently erasing them. The historical Yjs update log is still excluded; the backup stores the latest server-materialized document state and restore creates a fresh collaboration generation.
