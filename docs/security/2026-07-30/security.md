...
Each exported attachment-upload file, custom page-cover, and uploaded custom-icon entry is recorded with its byte size, CRC-32, and SHA-256 digest. Restore validates the ZIP directory, manifest relationships, entry paths, counts, media signatures, and digests before replacing workspace data. Current v3 backups enumerate both the account's attachment-upload directory and complete uploaded custom-icon directory rather than only DB-referenced active assets. Extra attachment files deliberately retained after an ambiguous database commit are recorded as `retainedAttachments`, while icon files intentionally retained after library removal remain self-contained in `custom-icons/`.
...
The restore preflight fingerprint and locked recheck also include the live attachment/custom-icon filesystem generations plus custom-icon library rows/removal rows, so a newer upload or icon-library mutation cannot be silently overwritten by a restore prepared from older destination state.
...
Restore is intentionally destructive for workspace content: current pages, collections, blocks, tag links, page sharing grants, and the attachment directory are replaced by the backup state. Current v3 restores also replace the account's uploaded custom-icon directory and custom-icon library/removal state; older backups leave that newer state untouched. Local uploaded-icon URLs and their removal hashes are rebound from the source account ID to the destination account ID. Login credentials and MFA/passkey security material are not exported and remain unchanged.
