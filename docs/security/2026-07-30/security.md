...
Restore is intentionally destructive for workspace content: current pages, collections, blocks, tag links, page sharing grants, user-visible page Version history, owned-page navigation collapse state, and the attachment directory are replaced by the current v4 backup state. v4 retains v3 uploaded-asset behavior and replaces the account's uploaded custom-icon directory plus custom-icon library/removal state; backups before v3 leave that newer asset state untouched. Local uploaded-icon URLs and their removal hashes are rebound from the source account ID to the destination account ID, and v4 applies the same rebinding to local icon references and the source-owner actor ID stored in page Version history. Login credentials and MFA/passkey security material are not exported and remain unchanged.
