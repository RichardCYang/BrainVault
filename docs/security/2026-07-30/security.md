...
Each exported attachment-upload file, custom page-cover, and uploaded custom-icon entry is recorded with its byte size, CRC-32, and SHA-256 digest. Restore validates the ZIP directory, manifest relationships, entry paths, counts, media signatures, and digests before replacing workspace data. Current v5 backups preserve user-visible page Version history, page/collection sharing state, page comments, and owned-page navigation state with a stricter complete-manifest contract. Their uploaded-asset handling retains the v3 behavior: both the account's attachment-upload directory and complete uploaded custom-icon directory are enumerated rather than only DB-referenced active assets. Extra attachment files deliberately retained after an ambiguous database commit are recorded as `retainedAttachments`, while icon files intentionally retained after library removal remain self-contained in `custom-icons/`.

Files are staged first, and the database replacement runs inside a transaction so a malformed or incomplete backup does not partially overwrite the account. Attachments and uploaded custom icons use one user-row lock and a shared restore journal: their previous directories are preserved until the database commit outcome is known, and interrupted rollback preserves later concurrent uploads that do not belong to the failed restore generation.
The restore preflight fingerprint and locked recheck also include page Version history, owned-page navigation collapse state, the live attachment/custom-icon filesystem generations, and custom-icon library rows/removal rows, so a newer history reset, navigation preference write, upload, or icon-library mutation cannot be silently overwritten by a restore prepared from older destination state. Navigation preference mutations take the same per-user row lock used by backup/restore.
...
Restore is intentionally destructive for workspace content: current pages, collections, blocks, tag links, page sharing grants, user-visible page Version history, owned-page navigation collapse state, and the attachment directory are replaced by the current v5 backup state. v5 retains v3 uploaded-asset behavior and replaces the account's uploaded custom-icon directory plus custom-icon library/removal state; backups before v3 leave that newer asset state untouched. Local uploaded-icon URLs and their removal hashes are rebound from the source account ID to the destination account ID, and v5 applies the same rebinding to local icon references and the source-owner actor ID stored in page Version history. Login credentials and MFA/passkey security material are not exported and remain unchanged.
...
Backup manifests are bounded to 20,000 pages, 50,000 blocks, 20,000 tags, 100,000 page-tag relations, 20,000 sharing grants, 200,000 page Version-history entries, 20,000 owned-page navigation-collapse entries, 5,000 uploaded attachment files total (live-block attachments plus retained unlinked files), 20,000 custom page-cover entries, 20,000 uploaded custom-icon files, and 50,000 custom-icon library-removal records. Import accepts at most 45,001 ZIP entries, including the manifest, and at most an 8 MiB central directory. BrainVault accepts only its UTF-8, store-mode ZIP format, so compressed-entry expansion is not available as an import path.
