...
The ZIP contains the account workspace graph: normal and archived pages, collections, nested blocks, block metadata, page/tag relationships, page sharing grants bound to collaborator account ID and username, supported profile preferences, every file physically retained in the account attachment-upload directory as its original byte sequence, custom page-cover bytes, and every uploaded custom image-icon file owned by the account. Current v3 exports record attachment files referenced by live blocks as normal `attachments` and any additional safely retained upload files as `retainedAttachments`; they also add digest-bound `custom-icons/` entries plus the custom-icon library/removal state, including uploaded icon files that were removed from the picker but are intentionally retained for immutable page references. Local `/upload/icons/<userId>/...` values are rebound to the importing account ID, so a backup created on a lost server can be restored into a newly created account whose internal ID differs. v2 custom-cover entries and v1 inline-cover backups remain importable. Before export, BrainVault coordinates same-origin tabs and refuses to create an incomplete archive while an owned page still has an unsaved direct draft or an unconfirmed real-time recovery snapshot in browser storage.
