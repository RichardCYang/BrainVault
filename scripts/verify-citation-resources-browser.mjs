...
// Usage: CHROMIUM_PATH=/path/to/chromium node scripts/verify-citation-resources-browser.mjs [--output file.json]
...
import { aiModuleSource, baseline } from '../tests/helpers/citation-tree-resource-harness.mjs';
...
async function browserChecks(modules) {
  ...
  // Real-DOM timings use prebuilt detached text nodes; do not include HTML parsing.
  const timings = [];
  const median = values => [...values].sort((a,b) => a-b)[Math.floor(values.length / 2)];
  for (const length of [50_000, 500_000]) {
    for (const operation of ['group', 'wrapper', 'hydrate']) {
      const node = document.createElement('p'); node.textContent = 'x'.repeat(length);
      const root = document.createElement('div');
      root.innerHTML = '<div class="rendered-ai-chat-answer"><div class="rendered-ai-chat-content"></div></div>';
      root.querySelector('.rendered-ai-chat-content').append(node);
      const preview = async () => { throw new Error('Plain paragraph must not fetch previews'); };
      const run = module => operation === 'hydrate' ? module.hydrateRenderedAiChatLinks(root, preview) : module[operation](node);
      for (let i = 0; i < 10; i++) { run(old); run(fixed); }
      const samples = { baseline: [], current: [] };
      // Batch the short optimized path separately to avoid reporting zero
      // milliseconds solely because performance.now() is privacy-coarsened.
      const calls = { baseline: length === 500_000 ? 8 : 25, current: operation === 'group' ? 10_000 : 1000 };
      for (let sample = 0; sample < 7; sample++) {
        const order = sample % 2 ? [['current', fixed], ['baseline', old]] : [['baseline', old], ['current', fixed]];
        for (const [mode, module] of order) {
          const before = performance.now();
          for (let i = 0; i < calls[mode]; i++) run(module);
          samples[mode].push((performance.now() - before) / calls[mode]);
        }
      }
      timings.push({ operation, length, callsPerSample: calls, samples,
  ...
  return { passed: checks.length, failed: 0, deterministicDomFixtures: fixtures.length, hydrationCases: htmlCases.length,
    checks, timings, userAgent: navigator.userAgent,
    scope: 'Native Chromium DOM, production ESM embedded as offline data URLs; AI hydration, editor, pagination and sanitizer. No authenticated full-app or database E2E. Timings exclude parsing/network; optimized paths use larger batches for clock resolution.' };
}
...
try {
  ...
  const result = { inputSha256: baseline.inputSha256, browser: version, ...evaluated.result.value };
