...
test("backup v4 preserves page version history and owned-page navigation state", async () => {
  ...
  assert.match(transfer, /FROM user_navigation_page_order no[\s\S]*?WHERE no\.user_id = \? AND p\.owner_id = \?/);
  ...
  assert.match(transfer, /navigationPageOrder: snapshot\.navigationPageOrder\.map/);
  ...
  assert.match(
    authRoutes,
    /\/navigation-order[\s\S]*?transaction\(async \(client\) => \{[\s\S]*?SELECT id FROM users WHERE id = \? FOR UPDATE/
  );
});
