...
const emojiRendererSource = fs.readFileSync(path.join(root, "public/emoji-renderer.js"), "utf8");
...
describe("page and collection emoji picker", () => {
  it("renders page, collection, search, category, and result controls", () => {
    ...
    expect(styles).toContain(".app-icon-emoji-image");
    expect(emojiRendererSource).toContain('const twemojiVersion = "17.0.3"');
    expect(emojiRendererSource).toContain("getTwemojiAssetKey");
    expect(emojiRendererSource).toContain('referrerPolicy = "no-referrer"');
    expect(app).toContain('from "./emoji-renderer.js"');
