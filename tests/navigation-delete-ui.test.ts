...
describe("Page and collection deletion menus", () => {
  ...
  it("supports click, touch-generated click, keyboard focus, outside dismissal, and permanent deletion", () => {
    ...
    expect(client).toContain('let task = findPendingPageDeleteTask(authenticationScope, target.id);');
    expect(client).toContain('const deletionSnapshot = await api(`/api/pages/${target.id}/deletion-snapshot`)');
    expect(client).toContain("return withPageEditLock(async () => {");
    ...
    expect(client).toContain('function findPendingPageDeleteTask(authenticationScope, pageId)');
    expect(client).toContain('function getPageDeleteTask(authenticationScope, pageId, expectedSnapshot, pageIds)');
    expect(client).toContain('function submitPageDeleteTask(task, authenticationScope)');
    expect(client).toContain('body: { expectedSnapshot: task.expectedSnapshot, mutationId: task.mutationId }');
    expect(client).toContain('isAmbiguousApiError(error)');
