...
describe("BrainVault web shell and health endpoint", () => {
  ...
  it("serves web UI assets", async () => {
    ...
    expect(response.text).toContain('body: { ...task.payload, expectedVersion: currentVersion, mutationId: task.mutationId }');
  ...
  it("includes migrations for the TABLE, KANBAN, DATABASE, Gantt, BOOKMARK, AI chat, and math block enums", async () => {
    const fs = await import("node:fs/promises");
    ...
    expect(baseline).toContain("'CALLOUT', 'TABLE', 'KANBAN', 'DATABASE', 'GANTT', 'BOOKMARK', 'AI_CHAT', 'MATH', 'CODE'");
    ...
    expect(ganttMigration).toContain("'GANTT'");
