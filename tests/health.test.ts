...
describe("BrainVault web shell and health endpoint", () => {
  ...
  it("serves web UI assets", async () => {
    ...
    expect(response.text).toContain('expectedVersion: currentVersion');
    expect(response.text).toContain('basePageContentVersion: task.basePageContentVersion');
  ...
  it("includes migrations for the TABLE, KANBAN, DATABASE, tree view, timetable, Gantt, BOOKMARK, AI chat, and math block enums", async () => {
    ...
    expect(baseline).toContain("'CALLOUT', 'TOGGLE', 'ACCORDION', 'TABLE', 'KANBAN', 'DATABASE', 'TREEVIEW', 'TIMETABLE', 'GANTT', 'BOOKMARK', 'AI_CHAT', 'MATH', 'MERMAID', 'CODE'");
