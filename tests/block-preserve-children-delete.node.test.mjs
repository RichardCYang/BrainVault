...
test("the UI sends one preserve-children deletion instead of committing a reorder first", () => {
  ...
  assert.match(
    emptyDelete,
    /deleteBlockWithVersionCheck\(blockId, \{\s*includeDescendants: false,\s*preserveChildren: true\s*\}\)/
  );
...
test("the server promotes children and deletes the target inside one locked SQL transaction", () => {
  ...
  assert.match(route, /expectedPageContentVersion: z\.number\(\)\.int\(\)\.min\(1\)\.optional\(\)/);
